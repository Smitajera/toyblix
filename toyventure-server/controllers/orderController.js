...
// @desc    Request item return or exchange
// @route   PUT /api/orders/:id/item/:itemId/return
// @access  Private
const requestItemReturn = async (req, res) => {
    try {
        const { type, reason, returnImage } = req.body;
        
        if (!returnImage) {
            return res.status(400).json({ message: 'A photo of the product is required for returns/exchanges' });
        }
        ...
        // Return policy check: must be delivered within 7 days
        if (order.orderStatus !== 'delivered' && order.orderStatus !== 'fulfilled') {
            return res.status(400).json({ message: 'Order must be delivered before requesting return/exchange' });
        }
        ...
        if (daysSinceDelivery > 7) {
            return res.status(400).json({ message: 'Return/Exchange window (7 days) has expired' });
        }
        ...
        item.returnStatus = type === 'exchange' ? 'Exchange Requested' : 'Return Requested';
        item.returnReason = reason;
        item.returnImage = returnImage;
        item.returnRequestedAt = new Date();

        await order.save();
        res.json({ message: `${type === 'exchange' ? 'Exchange' : 'Return'} requested successfully`, item });
    } catch (error) {
        console.error(error);
...
// @desc    Update return status for an item
// @route   PUT /api/orders/:id/item/:itemId/return-status
// @access  Private/Admin
const updateItemReturnStatus = async (req, res) => {
    try {
        ...
        await order.save();
        res.json({ message: `Return/Exchange marked as ${status}`, item });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Server Error' });
    }
...
module.exports = { createOrder, getMyOrders, getOrders, updateOrderStatus, downloadInvoice, requestItemReturn, updateItemReturnStatus };
